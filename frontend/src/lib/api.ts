...
export async function fetchWeather(
  ...
  try {
    ...
    const info = WMO_WEATHER[code] ?? { emoji: "🌡️", label: "날씨 정보를 확인했어요" };
    return { ...info, temp };
  } catch {
    return null;
  }
...
export async function reverseGeocode(lat: number, lng: number): Promise<string | null> {
  try {
    ...
    const data = await res.json();
    const { suburb, city_district, county, city, town, village } = data.address ?? {};
    const district = city_district ?? county ?? "";
    const neighbourhood = suburb ?? "";
    const cityName = city ?? town ?? village ?? "";
    return [cityName, district, neighbourhood].filter(Boolean).join(" ") || null;
  } catch {
    return null;
  }
}
